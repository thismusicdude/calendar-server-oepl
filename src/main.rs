...
fn get_abs_path(path: &str) -> Result<PathBuf> {
    // format!("Hello {}!", name)
    let mut html_dir = env::current_dir()
...
async fn api_set_calendars(form: web::Json<ServerConfigFile>) -> impl Responder {
    ...
    HttpResponse::Ok().body("set confg successul")
}
...
async fn paperdisplay() -> impl Responder {
    ...
    for event in next_events {
        ...
        json_vector.push(json!([{"text": [5, last_coord, format!["{} Uhr", event.date.time().to_string() ],"fonts/calibrib20",2,0]}]));
